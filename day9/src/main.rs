...
fn main() {
    ...
    let inputs : Vec<usize> = input.lines().filter(|l| !l.trim().is_empty()).map(|l| l.parse::<usize>()).flatten().collect();
    ...
    for i in 0..inputs.len() {
        for j in (i+1)..(inputs.len() - i + 1) {
            ...
            if sum == target {
                println!("Found {}", subseq.iter().min().unwrap() + subseq.iter().max().unwrap());
            }
            if sum > target {
                break;
            }
